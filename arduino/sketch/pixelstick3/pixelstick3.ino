...
#define BUTTON_RUN    A0
#define BUTTON_CONT   A1
...
void loop() {
  ...
  switch (modus) {
    case MODUS_OFF:
      if (button == BUTTON_RUN) {
        Serial.println("Modus Run");
        modus = MODUS_RUN;
      }
      if (button == BUTTON_CONT) {
...
int inputNumber(int& number) {
  ...
  while (!done) {
    ...
    bool up = analogRead(BUTTON_UP) < 500;
    bool down = analogRead(BUTTON_DOWN) < 500;
    bool ok = analogRead(BUTTON_SELECT) < 500;
...
int blinkFile(char* path, int sleep) {
  ...
  unsigned char* pixel = (unsigned char*) malloc(BUFFER * 4);
  if (pixel != NULL) {
  ...
  for(short x = 0; x < w && !stop; x++) { // Columns, width
    unsigned short bytesToRead = NUMPIXELS;
    unsigned short y = 0;
    while(bytesToRead > 0) {
      unsigned short toRead = min(bytesToRead, BUFFER);
//      Serial.println(bytesToRead);
//      Serial.println(y);
      pixFile.read(pixel, toRead*4);
      for(unsigned short p=0; p<toRead*4; p+=4) {
